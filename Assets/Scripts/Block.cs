...
public class Block {
    ...
    public virtual MeshData BlockData (Chunk chunk, int x, int y, int z, MeshData meshData) {
        ...
        if (!chunk.GetBlock (x, y + 1, z).IsSolid (Direction.South)) {
            meshData = FaceDataNorth (chunk, x, y, z, meshData);
        }

        if (!chunk.GetBlock (x, y + 1, z).IsSolid (Direction.North)) {
            meshData = FaceDataSouth (chunk, x, y, z, meshData);
        }

        if (!chunk.GetBlock (x, y + 1, z).IsSolid (Direction.West)) {
            meshData = FaceDataEast (chunk, x, y, z, meshData);
        }

        if (!chunk.GetBlock (x, y + 1, z).IsSolid (Direction.East)) {
